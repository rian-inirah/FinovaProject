...
import { TrendingUp, Download, Filter, IndianRupee, Package, BarChart3 } from "lucide-react";
...
const ItemReports = () => {
  ...
  const fetchReports = async () => {
    try {
      ...
      toast.error("Failed to fetch item reports");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const exportToCSV = () => {
    ...
    const csvData = [
      ...
      ...reports.itemReports.map((item) => [
        ...
        item.totalQuantity,
        item.totalAmount,
        item.averagePrice || (item.totalQuantity ? (item.totalAmount / item.totalQuantity).toFixed(2) : 0),
        item.orderCount,
      ]),
    ];
      ...
      <div className="card-content">
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{item.orderCount} order{item.orderCount !== 1 ? "s" : ""}</p>
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span className="font-bold text-lg text-green-600">{parseFloat(item.totalAmount).toFixed(2)}</span>
            </div>
          </div>
        </div>
        <div className="mt-4 pt-4 border-t border-gray-200 flex justify-between text-sm">
          ...
          <span className="font-medium">
            ₹{item.averagePrice ? parseFloat(item.averagePrice).toFixed(2) : (item.totalAmount / item.totalQuantity).toFixed(2)}
          </span>
        </div>
      </div>
    </div>
    ...
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center">
          <TrendingUp className="mr-2" size={24} /> Item Reports
        </h1>
        <p className="text-gray-600 mt-1">Analyze item-wise sales performance</p>
      </div>

      {/* Filters */}
      <div className="card mb-6">
        <div className="card-header">
          <h3 className="text-lg font-semibold flex items-center"><Filter className="mr-2" size={20} /> Filters</h3>
        </div>
        <div className="card-content">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="date" value={filters.toDate} onChange={e => handleFilterChange('toDate', e.target.value)} className="input" />
            </div>
            <div className="flex items-end space-x-2">
              <button onClick={applyFilters} disabled={isLoading} className="btn btn-primary flex-1">Apply Filters</button>
        ...
        <>
          {!reports?.itemReports?.length ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">No items were sold in the selected date range</p>
            </div>
          ) : (
